...
// copyResults is used by non Qry read requests below.
// References to db values are invalid outside transaction, so result values are copied.
func copyResults(results [][]byte) [][]byte {
	respRecs := make([][]byte, len(results))
...
func Get(tx *bolt.Tx, req *GetRequest) *Response {
	...
	results := make([][]byte, 0, len(req.Keys))
...
func GetAll(tx *bolt.Tx, req *GetAllRequest) *Response {
	...
	results := make([][]byte, 0, DefaultQryRespSize)
	...
	for k != nil {
		...
		results = append(results, v)
		if len(results) == req.Limit { // note - if limit == 0, len(result) cannot be zero when compare is made
			break
		}
		k, v = csr.Next()
	}
...
func GetIndex(tx *bolt.Tx, req *GetIndexRequest) *Response {
	...
	// NOTE - value of index record is key of record in data bkt
