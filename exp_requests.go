...
// GetValues returns specific values rather than entire record.
// Note - more extensive error handling is possible with fastjson, but not included in GetVals
func GetValues(tx *bolt.Tx, req *GetValuesRequest) *Response {
	...
	for _, key := range req.Keys {
		...
		for _, fld := range req.Fields {
			...
			switch fldType {
			case "string":
				if strBytes := parsedRec.GetStringBytes(fldName); strBytes != nil { // nil = problem
					fldVal = string(strBytes)
				}
			case "int":
				intVal := parsedRec.GetInt(fldName)
